...
exports.loadData = function(glob) {
  const parseData = async file => {
    ...
    if (file.extname === '.json') {
      ...
      data = {
        key: file.relative,
        value: yaml.load(file.contents.toString()),
      };
    }
...
exports.loadScreenshots = glob => {
  const parseFile = file => file.relative;

  const src = vfs.src(glob, { read: false });
  return new Promise((resolve, reject) => {
    stream.pipeline(
      src,
      concat(files => resolve(files.map(parseFile))),
      err => err && reject(err),
    );
  });
};
